<template>
  <div>
    <AppButton
      :href="`/dashboard/manage-template-tasks/${task.task_template?.id}/edit`"
    ...
    >
      ...
      <div class="flex justify-between">
        ...
        <div class="justify-end text-sm">{{ moment(task.created_at).format('MMM DD, YYYY hh:mm A') }}</div>
      </div>
      ...
      <div class="flex justify-end">
        <div v-if="task.task_user_access" class="text-sm flex justify-end">
          <img :src="task.task_user_access.user.avatar" class="w-3 h-3 inline mr-2 mt-[4px] rounded-full text-green-800" />
      ...
      <!-- NOTE: ROW FINAL -->
      <div v-if="task.message" class="flex bg-brand-50 px-4 py-2 rounded-2xl col-span-3 text-sm">
        {{ task.message }}
      </div>
    </AppButton>
  </div>
