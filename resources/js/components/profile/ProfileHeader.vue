<template>
  <div>
    <div class="hover:shadow-md rounded-b-3xl transition-all hover:opacity-80 z-0">
      <img @click="openCover = true" class="h-32 w-full object-cover lg:h-48 rounded-b-3xl" :src="user.cover" alt="" />
    </div>
    <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
      <div class="-mt-12 sm:-mt-16 sm:flex sm:items-end sm:space-x-5">
        <div class="flex">
          <img @click="openAvatar = true" class="hover:shadow-xl transition-all cursor-pointer h-24 w-24 rounded-full ring-4 ring-white sm:h-32 sm:w-32 hover:opacity-80" :src="user.avatar" alt="" />
        </div>
...
import { XMarkIcon, PaperAirplaneIcon, UsersIcon } from '@heroicons/vue/20/solid'
