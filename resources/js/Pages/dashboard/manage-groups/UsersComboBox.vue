<template>
  <Combobox as="div" @update:modelValue="value => AddMember(value)" class="gap-2">
    ...
    <div class="relative mt-2 flex-grow">
      ...
      <ComboboxInput class="shadow-inner w-full rounded-xl border-0 bg-white py-1.5 pl-3 pr-10 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-brand-600 sm:text-sm sm:leading-6" @change="search_input = $event.target.value" />
...
async function userSearch() {
  loading.value = true
  users_from_DB.value = await axios.get(`/dashboard/manage-groups/users-suggestion/${$props.groupId}`, {
